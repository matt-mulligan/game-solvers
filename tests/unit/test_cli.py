...
class TestWordleCLI:
    ...
    @mark.parametrize("input_value,expected", [
        ...
        ("s", ["S"]),
        ("v,e", ["V", "E"]),
        ("f,t,w,c,s,q,g,e,b,u,i,o,p", ["F", "T", "W", "C", "S", "Q", "G", "E", "B", "U", "I", "O", "P"]),
    ])
    ...
    def test_when_parse_grey_and_too_long_then_raise_error(self, wordle_cli):
        err_msg = re.escape(
            f"Each input for grey letters must a single letter. Input ge is invalid."
        )
    ...
    def test_when_solutions_then_correct_calls_made(self, wordle_cli):
        ...
        with patch("game_solvers.cli.Wordle") as wordle:
