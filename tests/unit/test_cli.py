...
class TestWordleCLI:
    ...
    def test_when_parse_green_then_correct_values_returned(
        self, wordle_cli, input_value, expected
    ):
    ...
    def test_when_parse_green_and_value_too_long_then_raise_error(self, wordle_cli):
        err_msg = re.escape(
            ...
            "Input a12 is invalid"
        )

        with raises(ValueError, match=err_msg):
            wordle_cli.parse_green("a12")

    def test_when_parse_green_and_first_char_not_letter_then_raise_error(
        self, wordle_cli
    ):
        err_msg = re.escape(
            "Each input for green letters must a letter followed by a number from 1-5. "
    ...
    def test_when_parse_green_and_second_char_not_valid_digit_then_raise_error(
        self, wordle_cli, input_value
    ):
        err_msg = re.escape(
            "Each input for green letters must a letter followed by a number from 1-5. "
    ...
    def test_when_parse_yellow_then_correct_values_returned(
        self, wordle_cli, input_value, expected
    ):
    ...
    def test_when_parse_yellow_and_input_wrong_size_then_raise_error(
        self, wordle_cli, input_value
    ):
        err_msg = re.escape(
    ...
    def test_when_parse_yellow_and_char_one_not_letter_then_raise_error(
        self, wordle_cli
    ):
        err_msg = re.escape(
    ...
    def test_when_parse_yellow_and_char_two_onwards_not_wordle_digit_then_raise_error(
        self, wordle_cli, input_value
    ):
        err_msg = re.escape(
            f"Each input for yellow letters must a letter followed by one to four numbers from 1-5. "
    ...
    def test_when_parse_grey_then_correct_values_returned(
        self, wordle_cli, input_value, expected
    ):
    ...
    def test_when_parse_grey_and_too_long_then_raise_error(self, wordle_cli):
        err_msg = re.escape(
            "Each input for grey letters must a single letter. Input ge is invalid."
        )
    ...
    def test_when_parse_grey_and_not_letter_then_raise_error(self, wordle_cli):
        err_msg = re.escape(
            "Each input for grey letters must a single letter. Input 4 is invalid."
        )
    ...
    def test_when_solutions_then_correct_calls_made(self, wordle_cli):
        ...
        with patch("game_solvers.cli.WordleSolver") as wordle:
